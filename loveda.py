...
class SMLoveDADataset(LoveDADataset):
    ...
    def load_annotations(self, img_dir, img_suffix, ann_dir, seg_map_suffix,
        ...
        if split is not None and osp.exists(split):
            ...
            print_log(f"Using split file: {split} with {len(split_ids)} images",
                    logger=get_root_logger())
        elif split is not None:
            print_log(f"WARNING: Split file {split} not found. Loading all images.",
                    logger=get_root_logger())

        for img in mmcv.scandir(img_dir, img_suffix, recursive=True):
            img_id = osp.splitext(osp.basename(img))[0]

            # check condition: if no split OR image is in split list
            if split_ids is None or img_id in split_ids:
                ...
                if ann_dir is not None:
                    ...
                    print_log(f"WARNING: No annotation dir found: {ann_dir}",
                            logger=get_root_logger())
